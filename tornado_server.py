...
class IndexHandler(web.RequestHandler):
    ...
    async def get(self):
        ...
        #     await self.render("index.html", temperature=0,
        #                       data2=e, humidity=0, date=0, devices=0)
            # await self.on_finish()
            # await self.finish()
    def prepare(self):
        database.connect()
                    ...
                    if(msg['device_name']+msg['class'] in info_dict.keys()):  # 判断是否已经连接,这里匹配，应该为 {'device_name': 'fan-lamp-curtain', 'class': 'G511', 'fan': '3', 'lamp': '3', 'curtain': '1'}
                        ...
                        if (msg == info_dict[msg['device_name']+msg['class']]):  # 判断是否心跳包
                            ...
                            if (msg['device_name'] == 'fan-lamp-curtain'):
                                ...
                                if('degree' and 'status' and 'gear' and 'model' in msg.keys()):
                                    logging.info('更新全部')
        ...
        if dicts['header']['namespace'] == "AliGenie.Iot.Device.Discovery":
            ...
            if name == 'TurnOn':
                ...
                if deviceId == 'fan':
                    ...
                    send_data = '''{'device_name': 'fan-lamp-curtain', 'class': 'G406', 'lamp-1': '1', 'lamp-2': '1','lamp-3': '1'}'''
                elif deviceId == 'alldevices':
                    send_data = '''{'device_name': 'fan-lamp-curtain', 'class': 'G406', 'fan-1': '1','fan-2': '1','fan-3': '1', 'lamp-1': '1', 'lamp-2': '1','lamp-3': '1', 'curtain-1': '1',}'''
            elif name == 'TurnOff':
                if deviceId == 'fan':
                    print('关')
                    ...
                    send_data = '''{'device_name': 'fan-lamp-curtain', 'class': 'G406', 'fan-1': '0','fan-2': '0','fan-3': '0', 'lamp-1': '0', 'lamp-2': '0','lamp-3': '0', 'curtain-1': '0', 'curtain-2': '0'}'''
            # try:
            print(send_data)
            send_data = eval(send_data)
            if isinstance(send_data, dict):
